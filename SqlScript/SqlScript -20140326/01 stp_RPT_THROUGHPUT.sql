	...
	BEGIN
		...
		BEGIN
			...
			WHERE	SC.SUBSYSTEM_TYPE=@SUBSYSTEM_TYPE
				...
				AND NOT EXISTS(SELECT LNST.DEVICE_LOCATION FROM #TRPT_FINALRESULT_TEMP LNST WHERE LNST.DEVICE_LOCATION=SC.DETECT_LOCATION)
				--AND SC.MDS_DATA=0
			GROUP BY SC.DETECT_LOCATION

			--2.2 count the total bags for normal subsystems by MDS_BAG_COUNT
			INSERT INTO #TRPT_FINALRESULT_TEMP
			SELECT	@STARTTIME_IDX AS STARTTIME,SC.DETECT_LOCATION, SUM(MBC.DIFFERENT) AS TOTAL_BAGS
			...
			WHERE	SC.SUBSYSTEM_TYPE=@SUBSYSTEM_TYPE
				...
				AND MBC.TIME_STAMP BETWEEN @STARTTIME_IDX AND @ENDTIME_IDX
				AND NOT EXISTS(SELECT LNST.DEVICE_LOCATION FROM #TRPT_FINALRESULT_TEMP LNST WHERE LNST.DEVICE_LOCATION=SC.DETECT_LOCATION)
				--AND SC.MDS_DATA=1 
			GROUP BY SC.DETECT_LOCATION
