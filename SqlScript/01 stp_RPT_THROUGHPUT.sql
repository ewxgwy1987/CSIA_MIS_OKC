	...
	BEGIN
		...
		BEGIN
			...
			WHERE	SC.SUBSYSTEM_TYPE=@SUBSYSTEM_TYPE
				...
				AND ITI.TIME_STAMP > @STARTTIME_IDX AND ITI.TIME_STAMP <= @ENDTIME_IDX 
				--AND SC.MDS_DATA=0
		...
		BEGIN
			...
			WHERE	SC.SUBSYSTEM_TYPE=@SUBSYSTEM_TYPE
				...
				AND IPR.TIME_STAMP > @STARTTIME_IDX AND IPR.TIME_STAMP <= @ENDTIME_IDX 
			...
			WHERE	SC.SUBSYSTEM_TYPE=@SUBSYSTEM_TYPE
				...
				AND MC.TIME_STAMP > @STARTTIME_IDX AND MC.TIME_STAMP <= @ENDTIME_IDX 
				AND NOT EXISTS(SELECT LNST.DEVICE_LOCATION FROM #TRPT_FINALRESULT_TEMP LNST WHERE LNST.DEVICE_LOCATION=SC.DETECT_LOCATION AND LNST.STARTTIME=@STARTTIME_IDX)
				--AND SC.MDS_DATA=1 
			GROUP BY SC.DETECT_LOCATION
