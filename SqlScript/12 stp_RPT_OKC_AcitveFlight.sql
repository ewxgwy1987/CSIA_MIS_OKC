...
BEGIN
	...
	INSERT INTO @SC_ALLOCINFO_TABLE
	SELECT DISTINCT TAF.AIRLINE,TAF.FLIGHT_NUMBER,TAF.STD,FPS.WEEKDAY
	FROM	#TMP_ACTIVE_FLIGHT TAF, FLIGHT_PLAN_SORTING FPS WITH(NOLOCK)
	WHERE	TAF.AIRLINE=FPS.AIRLINE AND TAF.FLIGHT_NUMBER=FPS.FLIGHT_NUMBER 
		AND TAF.STD=CONVERT(datetime,CONVERT(VARCHAR,FPS.SDO,103)+' '+DBO.RPT_GETFORMATTEDSTO(FPS.STO),103)
