...
ALTER PROCEDURE dbo.stp_RPT18_BAGNOTFOUND_GWYTEST
		  @DTFROM datetime, 
		  @DTTO datetime
AS
BEGIN
	...
	--2. Query bag data in ITEM_REDIRECT with No flight or No allocation
	...
	--3.Query Bag BSM Data and Item Scanned Data for each not found baggage
	SELECT	BSG.AIRLINE AS CARRIER_ID, 
			BSG.LICENSE_PLATE AS TAG_NUMBER,
			ISC.TIME_STAMP AS TIME_SEEN,
			LOC.LOCATION AS LOCATION_SEEN,
	...
	INNER JOIN #BNF_BAG_SORTING_TEMP BSG ON (BSG.LICENSE_PLATE=ISC.LICENSE_PLATE1 OR BSG.LICENSE_PLATE=ISC.LICENSE_PLATE2)
