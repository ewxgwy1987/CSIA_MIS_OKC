...
BEGIN
	...
	--INSERT	INTO #PTK_PECTRACKING_FINAL
	--SELECT	DISTINCT MBCR.SUBSYSTEM, LOC.LOCATION AS PhotoCellID,INTERNAL_LOC,0 AS TOTAL_BAG, 0 AS MISS, 0 AS UNKNOWN, 0 AS BJAM, 0 AS MSBJ
	--FROM	MDS_COUNTERS MBCR, LOCATIONS LOC
	--WHERE	MBCR.SUBSYSTEM IN (SELECT * FROM RPT_GETPARAMETERS(@SubSystem))
	--	AND	MBCR.SUBSYSTEM=LOC.SUBSYSTEM
	--	AND MBCR.LOCATION=LOC.LOCATION
	--	AND LOC.TRACKED=1 AND MBCR.TYPE='CV'
