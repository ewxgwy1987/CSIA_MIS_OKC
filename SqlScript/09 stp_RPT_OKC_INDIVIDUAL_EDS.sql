...
BEGIN
	...
	WHERE	DPM.EQUIP_SUBSYSTEM=@EDS_SUBSYSTEM 
	...
	--EDS Fault
	DECLARE @EQUIP_ID VARCHAR(20)='XR-'+@EDS_SUBSYSTEM;
	DECLARE @ESTP_EQUIPID VARCHAR(20)='CS-'+@EQUIP_ID+'-ES';

	--1. ESTOP 
	UPDATE	#INDIV_EDS_STATUS
	SET		ESTOP = dbo.RPT_GET_FAULT_DURATION(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@ESTP_EQUIPID,'AA_ESTP')

	--2. FAULTS
	UPDATE	#INDIV_EDS_STATUS
	SET		FAULTS = dbo.RPT_GET_FAULT_DURATION(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@EQUIP_ID,'AA_FT,AA_XMAL')

	--3. RTR_HIGH
	UPDATE	#INDIV_EDS_STATUS
	SET		RTR_HIGH = dbo.RPT_GET_FAULT_DURATION(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@EQUIP_ID,'AA_RDRV')

	--4. RTR_LOW
	--UPDATE	#INDIV_EDS_STATUS
	--SET		RTR_LOW = dbo.RPT_GET_FAULT_DURATION(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@EQUIP_ID,'AA_NRRV')
	UPDATE	#INDIV_EDS_STATUS
	SET		RTR_LOW = DATEDIFF(SECOND,@DTFROM,@DTTO)-RTR_HIGH -- AA_RDRV IS REMOVED

	--5. JAMS
	DECLARE @PRE_XM_LOCATION VARCHAR(20);
	DECLARE @EDS_LOCATION VARCHAR(20);
	DECLARE @POST_XM_LOCATION VARCHAR(20);

	SELECT	@PRE_XM_LOCATION=ELD.PRE_XM_LOCATION,
			@EDS_LOCATION=ELD.EDS_LOCATION,
			@POST_XM_LOCATION=ELD.POST_XM_LOCATION
	FROM	GET_RPT_EDS_LINE_DEVICE() ELD
	WHERE	ELD.SUBSYSTEM=@EDS_SUBSYSTEM

	UPDATE	#INDIV_EDS_STATUS
	SET		JAMS =	dbo.RPT_GET_FAULT_TIMES(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@PRE_XM_LOCATION,'AA_BJAM')
				  + dbo.RPT_GET_FAULT_TIMES(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@EDS_LOCATION,'AA_BJAM')
				  + dbo.RPT_GET_FAULT_TIMES(@DTFROM,@DTTO,@EDS_SUBSYSTEM,@POST_XM_LOCATION,'AA_BJAM')
	...
	-- use ITI instead of MDS_COUNT
	UPDATE #INDIV_EDS_STATUS
	SET BAGS_SEEN =  
	(SELECT COUNT(ITI.GID)
	FROM ITEM_TRACKING ITI, LOCATIONS LOC, GET_RPT_EDS_LINE_DEVICE() ELD
	WHERE ITI.TIME_STAMP BETWEEN @DTFrom AND @DTTo
		AND ITI.LOCATION=LOC.LOCATION_ID
		AND LOC.SUBSYSTEM=ELD.SUBSYSTEM AND ELD.SUBSYSTEM=@EDS_SUBSYSTEM
...
--DECLARE @DTFROM DATETIME='2014-05-05'
--DECLARE @DTTO DATETIME='2014-05-06'
--DECLARE @SUBSYSTEM VARCHAR(20)='ED1'
--EXEC stp_RPT_OKC_INDIVIDUAL_EDS @DTFROM,@DTTO,@SUBSYSTEM